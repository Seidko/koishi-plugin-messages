import { Context, Service, $, Schema, Update } from 'koishi'
import { MaybeArray, makeArray } from 'cosmokit'

declare module 'koishi' {
  interface Tables {
    '@seidko/messages@v1': Messages
  }
...
class MessageService extends Service {
  ...
  constructor(public ctx: Context, public config: MessageService.Config) {
    ...
    async function upsert(msg: MaybeArray<Update<Messages>>) {
      ...
      await ctx.database.upsert('@seidko/messages@v1', messages, ['channelId', 'messageId', 'platform', 'guildId'])
    }

    async function getFinalMessages(platform: string, guildId: string, selfId: string): Promise<Task[]> {
      const a = await ctx.database.select('@seidko/messages@v1')
    ...
    ctx.database.extend('@seidko/messages@v1', {
      id: {
        type: 'unsigned',
        length: 8,
      },
    ...
    ctx.on('message-deleted', async session => {
      ...
      await ctx.database.set('@seidko/messages@v1', { messageId: msg.id, channelId: session.channelId, platform: session.platform }, {
    ...
    ctx.on('message-updated', async session => {
      const msg = session.event.message
      await ctx.database.set('@seidko/messages@v1', { messageId: msg.id, platform: session.platform }, {
        content: msg.content,
        updatedAt: new Date(msg.updatedAt ?? Date.now()),
        edited: true,
      })
